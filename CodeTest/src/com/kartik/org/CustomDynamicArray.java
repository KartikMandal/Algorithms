...
public class CustomDynamicArray {
	...
	public void addElement(int a) {
		// compares if the number of elements is equal to the size of the array
		// or not
		if (count == sizeofarray) {
			// invoking the growSize() method that creates an array of double
			// size
			growSize();
		}
	...
	public void shrinkSize() {
		...
		int temp[] = null;
		if (count > 0) {
			// creates an array of the size equal to the count i.e. number of
			// elements the array have
			temp = new int[count];
	...
	public void removeElementAt(int index) {
		if (count > 0) {
			for (int i = index; i < count - 1; i++) {
				// shifting all the elements to the left from the specified
				// index
				array[i] = array[i + 1];
			}
			array[count - 1] = 0;
	...
	public static void main(String[] args) {
		CustomDynamicArray da = new CustomDynamicArray();
